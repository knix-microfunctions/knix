...
class Config(dict):
    ...
    def get_client(self):
        if not (self.mfn_url and self.mfn_user and self.mfn_password):
            ...
            return MfnClient(self.mfn_url,self.mfn_user,self.mfn_password,self.mfn_name)
...
>>>>>>> 4fb884868e9a741937bff77aa33dbfd2c652dd92
    """
    if file:
        try:
            wfd = file.read()
...
def get_workflow(config,workflow):
    ...
    eplen = 0
    for wf in wfs:
        table.append((wf.id,wf._name,wf._status,datetime.datetime.fromtimestamp(wf._modified).strftime("%Y-%m-%d %H:%M:%S"),wf._endpoint or ""))
        namelen = max(namelen,len(wf._name))
        eplen = max(eplen,len(wf._endpoint or ""))
